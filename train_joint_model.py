...
class JointFrameSeqModel(nn.Module):
    def __init__(self, hidden: int = 64, encoder_type: str = "spatial") -> None:
        super().__init__()
...
def train_joint(
    ...
    model = JointFrameSeqModel(hidden=64, encoder_type=encoder_type).to(device)
        ...
        for _ in range(steps):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp, dtype=torch.float16):
                ...
                try:
                    ...
                    center = xs.shape[1] // 2
                    loss_s = criterion_seq(logits_s[:, center], ys)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--seq-dirs', type=str, default=None, help='Comma-separated sequence directories (e.g., toi5443,toi5595,toi5944)')
    ...
    p.add_argument('--lam-frame', type=float, default=0.7)
    p.add_argument('--lam-seq', type=float, default=0.3)
    p.add_argument('--lr', type=float, default=1e-3)
    p.add_argument('--weight-decay', type=float, default=1e-4)
    p.add_argument('--img-size', type=int, default=128)
    p.add_argument('--encoder', type=str, choices=['basic', 'spatial'], default='spatial')
